...
- **Rotación de la clave de OpenRouter en producción**: la clave anterior vivía en una cuenta distinta a la que Jorge ve en su panel, así que se sustituyó por una nueva **con límite de gasto de 5 $** y se actualizó en los dos sitios donde vive (`.env` del perfil y variable del servicio en Render, comprobando huella tras el PUT), con redespliegue y `/health/ready` en `UP`. Verificado end-to-end: manual subido → `LISTO` con embeddings (dos veces, ~10 s) → chat con fuentes.
- **El chat RAG caía con 503 por la reserva de tokens**: la petición a OpenRouter no fijaba `max_tokens`, así que el proveedor reservaba el máximo del modelo (16.384 tokens en `gpt-4o-mini`) y respondía **402 Payment Required** ("you requested up to 16384 tokens, but can only afford 3633") en cuanto el saldo disponible no cubría esa reserva, aunque la respuesta real fuese de 200 tokens. El error se traducía a `llm_no_disponible` (503) y el chat quedaba muerto con cualquier saldo pequeño. Ahora la petición pide `mecania.chat.max-tokens` (**800 por defecto**, configurable) y hay `OpenRouterLlmServiceTest` con los dos casos (max_tokens acotado y 402 → `LlmException`). Tests: 237 → **239** (202 unitarios + 37 de integración).
- **Encuadre de coste documentado de cara al lector (ADR-011)**: Mecania se construye para costar 0 €/mes y servir a un solo usuario, y eso pasa a estar escrito como decisión, no como disculpa. El ADR-011 separa lo que se decide por presupuesto de lo que se decide por criterio de ingeniería (con las alternativas evaluadas: modelo de pago, modelo autoalojado, embeddings locales), declara los límites asumidos y fija la señal de revisión. El README estrena la sección «Cómo está construido y cómo lo construiría con presupuesto», con las diez partidas que cambiarían (modelo de IA, embeddings, base de datos, almacenamiento, cómputo, operación, esquema, límite de peticiones, secretos, autenticación) y la lista explícita de lo que no cambia: arquitectura, 239 tests, contrato de errores, aislamiento entre usuarios y verificación en producción.
- **El chat pasa a un modelo gratuito** (`MECANIA_CHAT_MODEL=nvidia/nemotron-3-ultra-550b-a55b:free`), elegido por configuración y verificado en producción con el flujo completo. Los embeddings se mantienen de pago a propósito (no hay equivalente gratuito con la misma dimensión y el coste es de céntimos al año). Candidatos descartados tras probarlos: dos devolvían contenido vacío, uno exponía su razonamiento interno y otro venía limitado por el proveedor de origen.
