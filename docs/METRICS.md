...
## Resumen
El proyecto Mecania cuenta con **26 tests automatizados** que cubren los siguientes aspectos críticos del dominio:
...
- **10 tests**: validan subida, listado, descarga y transiciones de estado.
  - `subirDocumento(Long, MultipartFile)`: válido PDF, válido TXT, extensión no soportada, archivo demasiado grande, vehículo inexistente.
  - `findByVehiculoId(Long)`: devuelve la lista del repositorio.
  - `findByIdAndVehiculoId(Long, Long)`: existente y no-pertenece-al-vehículo.
  - `marcarComoListo(Long, Long)`: cambia estado correctamente.
  - `marcarComoError(Long, Long, String)`: cambia estado y mensaje de error.

### Capa de infraestructura (AlmacenamientoDiscoLocalTest)
- **4 tests**: validan el contrato de la implementación de almacenamiento.
  - `guardarArchivo(MultipartFile, String)`: escribe contenido y devuelve ruta relativa (con y sin subdirectorio).
  - `leerArchivo(String)`: devuelve bytes correctos.
  - `eliminarArchivo(String)`: borra el archivo físicamente.
...
- **Manejo de excepciones**: conversión de excepciones de dominio a códigos HTTP adecuados (404, 409, 400).
...
- **Almacenamiento físico**: lectura, escritura y borrado de archivos reales en disco temporal (no se mockea el filesystem).
- **Tipos de archivo**: validación de extensión (PDF, TXT, DOCX) y límite de tamaño (10 MB).
- **Aislamiento por vehículo**: los endpoints de documento exigen que el documento pertenezca al vehículo de la URL.
...
- **Tests de integración con Testcontainers para PostgreSQL**: las dependencias están declaradas pero no se usan todavía porque las pruebas actuales son unitarias con mocks o H2. Se activarán cuando se pruebe el camino completo BD + storage.
